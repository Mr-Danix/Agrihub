...
function getFarmerProducts() {
    ...
    const normalized = saved.map((p) => {
        const product = {
            ...
            origin: p.origin || 'Local Farm',
            icon: p.icon || '🌾'
        };
        if (!p.id) changed = true;
        return product;
    });
    if (changed) {
        localStorage.setItem('farmerProducts', JSON.stringify(normalized));
    }
    return normalized;
}

function getMarketplaceInventory() {
    return [...inventory, ...getFarmerProducts()];
}
...
function proceedToCheckout() {
    ...
    // Calculate the total price from the agriCart array
    const total = cart.reduce((sum, item) => sum + item.price, 0);

    // Save the total separately so the delivery page can grab it easily
    localStorage.setItem('agriTotal', total);
...
function addToCart(id) {
    ...
    const item = getMarketplaceInventory().find(p => p.id === id);
    if (!item) {
...
function filterProducts(searchTerm) {
    const allProducts = getMarketplaceInventory();
    if (!searchTerm.trim()) {
        return allProducts;
    }
